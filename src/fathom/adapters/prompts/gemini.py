        ...
        if sub_goal_info and isinstance(sub_goal_info, dict):
            ...
            if index is not None and total > 1:
                ...
                parts.append(
                    ...
                    f"6. WHEN TO SET sub_goal_completed (CRITICAL):\n"
                    f"   ONLY set 'sub_goal_completed: true' on the action that DIRECTLY "
                    f"fulfills the sub-goal's objective. Ask yourself: 'Does this specific "
                    f"action complete the task described in the sub-goal?'\n"
                    f"   YES — set sub_goal_completed:\n"
                    f"   - Task says 'select first result' → tap on the result item\n"
                    f"   - Task says 'search for X' → tap search/submit after typing X\n"
                    f"   - Task says 'add item to cart' → tap the Add/+ button\n"
                    f"   - Task says 'open app' → tap the app icon\n"
                    f"   - Task says 'set location' → tap the confirm/save location button\n"
                    f"   - Task says 'swipe left to reach X section' → the swipe that reveals X\n"
                    f"   - Task says 'type password' → the type action entering the password\n"
                    f"   - Task says 'enter address' → the type action entering the address\n"
                    f"   NO — do NOT set sub_goal_completed:\n"
                    f"   - Tapping a search field to focus it (preparatory step)\n"
                    f"   - Typing text when a submit/confirm step still follows\n"
                    f"   - Dismissing popups, overlays, permission dialogs, banners\n"
                    f"   - Scrolling/swiping to find an element (not the goal itself)\n"
                    f"   - Tapping back/home for navigation recovery\n"
                    f"</CURRENT_STEP>"
                )
                logger.debug(
