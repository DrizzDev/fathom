...
class TaskRepository:
    """
    Task repository: persists and transitions tasks within threads.
    """

    def __init__(self, *, context: StoreContext) -> None:
        """
        Bind shared store context for task persistence.
        """
    ...
    async def finish_task(self, *, request: FinishTask) -> Task:
        ...
        async with self.__context.unit.session() as connection:
            ...
            tasks_table = tables.TASKS
            statement = (
                SQLLiteQuery.update(tasks_table)
