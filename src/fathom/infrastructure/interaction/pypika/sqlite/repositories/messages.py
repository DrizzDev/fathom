...
class MessageRepository:
    """
    Message repository: persists, sanitizes, and lists thread messages.
    """

    def __init__(self, *, context: StoreContext) -> None:
        """
        Bind shared store context for message persistence.
        """
    ...
    async def record_message(self, *, request: RecordMessage) -> Message:
        ...
        Classification happens before the request crosses the interaction
        port; repositories persist labels and do not make policy decisions.
        """
        ...
        if query.task is None:
            async with (
                ...
                connection.execute(
                    ...
                    SELECT *
                    FROM messages
                    WHERE tenant = ? AND thread = ? AND deleted_at IS NULL
            ...
            async with (
                ...
                connection.execute(
                    ...
                    SELECT *
                    FROM messages
                    WHERE tenant = ? AND thread = ? AND task = ? AND deleted_at IS NULL
                    ORDER BY sequence ASC
                    """,
    ...
    def __same_record_message(self, *, message: Message, request: RecordMessage) -> bool:
        """
        Check whether a message request replays an already stored message.

        Content equality compares labels as a set so that replays with the
        same labels in a different declaration order are recognized as the
        same payload (Pydantic tuple equality is order-sensitive, which
        previously caused spurious "different content" rejections).
        """
        ...
        return (
            ...
            and self.__same_content(stored=message.content, target=request.content)
        )
    ...
    def __same_record_message_input(self, *, message: Message, request: RecordMessage) -> bool:
        ...
        The application service applies policy labels before crossing the port, so the repository compares the final content shape.
        """
