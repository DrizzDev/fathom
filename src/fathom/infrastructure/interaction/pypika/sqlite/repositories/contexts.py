...
class ContextRepository:
    """
    Context repository: persists reference-based context recipes.
    """

    def __init__(self, *, context: StoreContext) -> None:
        """
        Bind shared store context for context-recipe persistence.
        """
    ...
    async def build_context(self, *, request: BuildContext) -> Context:
        ...
        async with self.__context.unit.session() as connection:
            ...
            statement = (
                SQLLiteQuery.into(contexts_table)
                .columns(
                    contexts_table.id,
            ...
            # SQLite also recognizes `references` as reserved in some contexts; quote.
    ...
    async def get_contexts(self, *, query: ContextQuery) -> List[Context]:
        ...
        contexts_table = tables.CONTEXTS
        statement = (
            SQLLiteQuery.from_(contexts_table)
