...
class CleanupService:
    """
    Cleanup service: retention sweeps for expired requests, jobs, events, and soft deletes.
    """

    def __init__(self, *, context: StoreContext) -> None:
        """
        Bind shared store context for retention sweeps.
        """
    ...
    async def __cleanup_terminal_jobs(
        ...
        *,
        ...
        statement = (
            SQLLiteQuery.from_(jobs).delete().where(PypikaTuple(jobs.tenant, jobs.id).isin(victims))
        )
        sql, parameters = binder.render(query=statement)
        result = await connection.execute(sql, parameters)
        return max(result.rowcount or 0, 0)
    ...
    async def __purge_soft_deleted_tasks(
        ...
        *,
        ...
        """
        Physically delete soft-deleted tasks no longer referenced (single statement).
        """
        ...
        victims = (
            ...
            .where(
                Not(
                    ExistsCriterion(
                        SQLLiteQuery.from_(child_tasks)
                        .select(1)
        ...
        *,
        ...
        victims = (
            ...
            .where(
                Not(
                    ExistsCriterion(
                        SQLLiteQuery.from_(child_messages)
                        .select(1)
    ...
    async def __purge_soft_deleted_threads(
        ...
        *,
        ...
        for row in rows:
            ...
            version_count = await self.__delete_by_thread(
                connection=connection,
                table=tables.SCRIPT_VERSIONS,
                tenant=row_tenant,
                thread=thread,
            )
            script_versions_deleted += max(version_count, 0)
            script_count = await self.__delete_by_thread(
                connection=connection,
                table=tables.SCRIPTS,
                tenant=row_tenant,
                thread=thread,
            )
            scripts_deleted += max(script_count, 0)
            membership_count = await self.__delete_by_thread(
                connection=connection,
                table=tables.MEMBERSHIPS,
                tenant=row_tenant,
                thread=thread,
            )
            memberships_deleted += max(membership_count, 0)
            context_count = await self.__delete_by_thread(
                connection=connection,
                table=tables.CONTEXTS,
                tenant=row_tenant,
                thread=thread,
            )
            contexts_deleted += max(context_count, 0)
            job_count = await self.__delete_by_thread(
                connection=connection,
                table=tables.JOBS,
                tenant=row_tenant,
                thread=thread,
            )
            jobs_deleted += max(job_count, 0)
            event_count = await self.__delete_by_thread(
                connection=connection,
                table=tables.EVENTS,
                tenant=row_tenant,
                thread=thread,
            )
            events_deleted += max(event_count, 0)
            sequence_count = await self.__delete_by_thread(
                connection=connection,
                table=tables.SEQUENCES,
                tenant=row_tenant,
                thread=thread,
            )
            sequences_deleted += max(sequence_count, 0)
            thread_count = await self.__delete_thread_row(
                connection=connection,
                tenant=row_tenant,
                thread=thread,
            )
            threads_deleted += max(thread_count, 0)
