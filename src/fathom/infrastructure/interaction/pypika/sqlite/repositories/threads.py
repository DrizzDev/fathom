...
class ThreadRepository:
    """
    Thread repository: persists and queries durable interaction threads.
    """

    def __init__(self, *, context: StoreContext) -> None:
        """
        Bind shared store context for thread persistence.
        """
    ...
    async def set_thread_title(self, *, request: SetThreadTitle) -> Thread:
        ...
        Idempotent: a non-null stored title is left unchanged and the
        existing thread is returned. The host calls this to auto-fill the
        title from the first crawler-intent payload without trampling a title that another writer already set.
        """
