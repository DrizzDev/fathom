...
class PostgresArtifactRepository:
    """
    Postgres artifact repository: persists and lists artifact references.
    """

    def __init__(self, *, context: PostgresContext) -> None:
        """
        Bind shared Postgres context for artifact persistence.
        """
    ...
    async def link_artifact(self, *, request: LinkArtifact) -> Artifact:
        ...
        async with self.__context.unit.session() as connection:
            ...
            statement = (
                PostgreSQLQuery.into(artifacts)
                .columns(
                    artifacts.id,
                    artifacts.tenant,
                    artifacts.workspace,
