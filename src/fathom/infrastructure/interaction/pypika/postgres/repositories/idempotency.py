...
class PostgresIdempotencyRepository:
    """
    Postgres idempotency repository: tracks request hashes and terminal outcomes.
    """

    def __init__(self, *, context: PostgresContext) -> None:
        """
        Bind shared Postgres context for requests persistence.
        """
        ...
        async with self.__context.unit.session() as connection:
            ...
            if existing is not None:
                if request.created >= existing.expires:
                    delete_binder = ParameterizedQuery(parameter_style=SqlParameterStyle.NUMBERED)
    ...
    async def finish_request(self, *, request: FinishRequest) -> Idempotency:
        ...
        async with self.__context.unit.session() as connection:
            ...
            requests = tables.REQUESTS
            statement = (
                PostgreSQLQuery.update(requests)
