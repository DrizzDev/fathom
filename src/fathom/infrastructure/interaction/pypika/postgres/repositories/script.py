...
from pypika import Order, PostgreSQLQuery
...
class PostgresScriptRepository:
    """
    Postgres script repository: persists live scripts and immutable versions.
    """

    def __init__(self, *, context: PostgresContext) -> None:
        """
        Bind shared Postgres context for script persistence.
        """
    ...
    async def __apply_replay(
        ...
        *,
        ...
        existing = await self.__lock_script(
            connection=connection,
            script=request.identity.id,
            tenant=request.identity.tenant,
        )
    ...
    async def __apply_update(
        ...
        *,
        ...
        statement = (
            PostgreSQLQuery.update(scripts)
    ...
    async def __insert_version(
        ...
        *,
        ...
        statement = (
            PostgreSQLQuery.into(script_versions)
            .columns(
                script_versions.id,
    ...
    async def __lock_script(
        ...
        *,
        ...
        """
        Load one script row holding a row-level write lock for the active transaction.
        """
