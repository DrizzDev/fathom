...
class CommandGate:
    ...
    def validate(self, *, command: ToolCommand) -> AcceptedCommand:
        """
        Return a structurally-accepted command or raise structured model feedback.

        The gate validates only command structure (available action, required fields, target
        grounding). Completion authority and requirement admission live in the advancement
        policy and the pre-dispatch requirement binding, never here.
        """
    ...
    def __error(*, message: str) -> ToolValidationError:
        ...
        return ToolValidationError(
            ToolErrorFeedback(
                message=message, tool_call_id=None, tool_name="execute_ui", error_kind="validation"
            )
        )
