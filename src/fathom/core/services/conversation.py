...
from fathom.core.exceptions import (
    ConversationSummaryLimitExceeded,
    InteractionError,
    ThreadNotFoundError,
)
...
class ConversationService:
    ...
    def __init__(
        ...
        *,
        ...
        """
        Initialize the service with a durable ledger port and a signer.
    ...
    async def actor(
        ...
        actor = await self.__interaction.create_actor(
    ...
    async def create(
        ...
        async with self.__interaction.atomic():
            ...
            thread = await self.__interaction.create_thread(
    ...
    async def start(
        ...
        task = await self.__interaction.open_task(
            request=InteractionSchemas.OpenTask(
                ...
                plan=InteractionSchemas.Plan(
                    objective=request.objective, reference=request.reference
                ),
                created_at=request.created,
                metadata=InteractionSchemas.Metadata(entries=request.metadata),
            )
    ...
    async def finish(
        ...
        task = await self.__interaction.finish_task(
    ...
    async def save(self, *, request: ConversationSchemas.ScriptSave) -> InteractionSchemas.Script:
        ...
        return await self.__interaction.save_script(
    ...
    async def __transition(
        ...
        *,
        ...
        thread = await self.__interaction.transition(
    ...
    async def save_policy(self, *, request: InteractionSchemas.SavePolicy) -> None:
        ...
        await self.__interaction.save_policy(request=request)
    ...
    async def title(
        ...
        *,
        ...
        """
        Set a thread title only when the stored title is null. Idempotent —
        a non-null title is preserved. The host calls this on the first
        crawler-intent for a fresh conversation so the chat UI gets a
        title without the client having to supply one.
        """

        result = await self.__interaction.set_thread_title(
    ...
    async def list(
        ...
        state = self.__thread_state(value=query.state)
        page = await self.__interaction.list_threads(
            query=InteractionSchemas.ThreadListQuery(
                ...
                include_archived=self.__should_include_archived(state=state),
            )
        )
    ...
    async def messages(
        ...
        await self.__thread(
            query=ConversationSchemas.ConversationThreadQuery(
                tenant=query.tenant, thread=query.thread
            )
        )
    ...
    async def artifacts(
        ...
        page = await self.__interaction.list_artifacts(
    ...
    async def script(
        ...
        task = await self.__interaction.get_task(
            query=InteractionSchemas.TaskOneQuery(tenant=query.tenant, task=query.task)
        )
        if task is None or task.thread != query.thread:
            return None

        page = await self.__interaction.list_scripts(
    ...
    async def timeline(
        ...
        if self.__include_timeline_kind(query=query, kind=EntryKind.MESSAGE):
            message_page = await self.__interaction.list_messages(
                query=InteractionSchemas.MessageCursorQuery(
                    task=query.task,
                    order=query.order,
                    until=query.until,
        ...
        if self.__include_timeline_kind(query=query, kind=EntryKind.EVENT):
            event_page = await self.__interaction.list_events(
        ...
        if self.__include_timeline_kind(query=query, kind=EntryKind.ARTIFACT):
            artifact_page = await self.__interaction.list_artifacts(
                query=InteractionSchemas.ArtifactCursorQuery(
                    task=query.task,
                    order=query.order,
                    since=query.since,
        ...
        if self.__include_timeline_kind(query=query, kind=EntryKind.CONTEXT):
            context_page = await self.__interaction.list_contexts(
        ...
        return view.model_copy(update={"entries": tuple(signed_entries)})
    ...
    def __decode_timeline_cursor(self, *, value: str | None) -> CompositeTimelineCursor:
        """
    ...
    async def tasks(
        ...
        When `query.task` is supplied, the result is the subtree rooted at
        that task (the named task plus its descendants). Otherwise every
        root-level task in the thread is returned.
        """
        ...
        all_tasks = await self.__interaction.get_tasks(
    ...
    async def state(self, *, tenant: str, thread: str, task: str) -> str | None:
        """
        Return one task's state without loading the whole conversation tree.
        """

        stored = await self.__interaction.get_task(
            query=InteractionSchemas.TaskOneQuery(tenant=tenant, task=task)
        )

        if stored is None or stored.thread != thread:
            return None
    ...
    def __scoped_tasks(
        ...
        """
        When a subtree root is named, prune to that root plus its transitive
        descendants. Otherwise pass every task through unchanged.
        """
    ...
    async def __thread(
        ...
        thread = await self.__interaction.get_thread(
            query=InteractionSchemas.ThreadQuery(tenant=query.tenant, thread=query.thread)
        )
        if thread is None:
    ...
    def __thread_state(self, *, value: str | None) -> ThreadState | None:
        """
        Parse an optional thread-state filter.
        """
        ...
        try:
            ...
            raise InteractionError(f"Unknown conversation state '{value}'.") from exception
    ...
    def __should_include_archived(self, *, state: Optional[ThreadState]) -> bool:
        ...
        return state is ThreadState.ARCHIVED
    ...
    def __task_roots(
        self, *, tasks: Tuple[InteractionSchemas.Task, ...], scoped_root: Optional[str] = None
    ...
    def __artifact_view(
        self, *, artifact: InteractionSchemas.Artifact
    ) -> ConversationSchemas.ArtifactView:
        """
        Convert a durable artifact into a client-facing artifact row without signing.
        Use `__signed_artifact_view` on read paths that must hand the consumer a fetchable URI.
        """
