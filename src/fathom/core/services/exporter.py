...
class ScriptExporter:
    ...
    def __find_app_launch_boundary(
        ...
        for iteration, step in enumerate(iterable=steps):
            if iteration > max_launch_steps:
                return 0
    ...
    def export(
        ...
        *,
        ...
        while i < n:
            ...
            description = Normalizer.action(action_type=action_type_val, target=target, text=text)
    ...
    def __infer_wait_subject(rationale: Optional[str]) -> str:
        ...
        return Normalizer.wait(condition="screen to load", rationale=rationale) or "screen to load"
    ...
    def __get_condition(step: Union[StepResult, Dict[str, Any]]) -> Optional[str]:
        ...
        if action_type == "wait":
            condition = Normalizer.wait(condition=condition, rationale=rationale)
