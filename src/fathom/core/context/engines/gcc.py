...
class GitContextEngine(ContextEngine):
    ...
    def __init__(self) -> None:
        ...
        self.__branches: Dict[str, BranchState] = {
            "main": BranchState(name="main")
        }
    ...
    async def hydrate(self, *, data: Dict[str, Any]) -> None:
        ...
        self.__commit_nodes = {
            k: CommitNode(**v) for k, v in data.get("commits", {}).items()
        }
        self.__branches = {
            k: BranchState(**v) for k, v in data.get("branches", {}).items()
        }
        self.__current_branch = data.get("current", "main")
        self.__shadow_buffer = [
            ExecutionRecord(**r) for r in data.get("shadow", [])
        ]
