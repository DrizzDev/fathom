...
class GeminiExportPromptBuilder(ExportPromptBuilder):
    ...
    def build_system_instruction(self) -> str:
        ...
        return (
            ...
            "- }\n"
            "- Inside IF blocks: indent action/validation lines (Tap on..., Type '...' into..., Wait..., Scroll..., Swipe..., Validate..., Press...).\n"
            "Rules:\n"
            ...
            "8) The final non-structural line is final_validation: a single concise UI state assertion (what screen, page, or primary content is visible or displayed), inferred from intent, trace, and baseline. Align with the user's goal semantically but abstract procedural wording into an end-state phrase—not a generic placeholder and not a verbatim copy of a long imperative intent.\n"
            ...
            "18) final_validation must not restate imperative steps already covered by catalog actions—avoid click/tap/type/select/navigate/search-for phrasing and chained 'and then' procedures in that line.\n"
            "19) Use action_validations for state checks tied to specific earlier actions (e.g. list visible after search); use final_validation only for the terminal UI state after the last catalog action."
        )
    ...
    def build_user_prompt(
        ...
        *,
        ...
        return (
            ...
            "- final_validation must be one short line starting with 'Validate' that names the destination UI state as visible or displayed (e.g. experience details page, search results, cart), derived from what the last catalog actions achieved.\n"
            "- If the user intent is procedural, abstract to the resulting screen or primary object—do not repeat tap/click/type steps that already appear as catalog actions.\n"
            "- Example: after tapping a specific list item, prefer 'Validate that the experience details page is visible' over restating list-and-click instructions.\n"
