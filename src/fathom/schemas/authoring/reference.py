...
class CommandExample(SealedModel):
    ...
    flow: str = Field(min_length=1, description="Rendered Flow pattern for the situation.")
...
DRIZZ_COMMANDS: Tuple[CommandDoc, ...] = (
    ...
    CommandDoc(
        ...
        examples=(
            CommandExample(
                ...
                flow="Tap on first search result for ghar soap",
                reason="The target combines relative selection with the query that made it meaningful.",
            ),
            CommandExample(
                kind=AuthoringExampleKind.AVOID,
                situation="A recorded target says only first product card.",
                flow="Tap on first product card",
    ...
    CommandDoc(
        ...
        examples=(
            CommandExample(
                kind=AuthoringExampleKind.PREFERRED,
                situation="Scrolling stopped when a product reviews section became visible.",
                flow='Scroll down until "Ratings and Reviews section"',
                reason="The stop target is concrete and visible.",
            ),
            CommandExample(
                kind=AuthoringExampleKind.AVOID,
                situation="Several scroll attempts mention broad list context.",
                flow='Scroll down until "product list"',
    ...
    CommandDoc(
        ...
        examples=(
            CommandExample(
                kind=AuthoringExampleKind.PREFERRED,
                situation="Evidence captured value 76 as item_price.",
                flow="Store 76 as item_price",
                reason="The command stores the actual runtime value.",
            ),
            CommandExample(
                kind=AuthoringExampleKind.AVOID,
                situation="Evidence captured value 76 for the product price.",
                flow="Store product price as item_price",
    ...
    CommandDoc(
        ...
        examples=(
            CommandExample(
                kind=AuthoringExampleKind.PREFERRED,
                situation="A checkout screen with phone number input is visible.",
                flow="Validate Phone Number input field is visible",
            ...
            CommandExample(
                ...
                flow="Validate NONE OF THE ABOVE button is visible",
