...
class CaseAttempt(SealedModel):
    ...
    assertion: NonBlank = Field(
        description="Exact assertion threaded into the prompt this attempt."
    )
    expected_verdict: VisualVerdict = Field(description="Oracle verdict for an observed goal.")
    expected_admission: bool = Field(
        description="Whether the effective rule should advance this attempt."
    )
    verdict: Optional[VisualVerdict] = Field(
        default=None, description="Model verdict, or None when absent."
    )
    confidence: Optional[float] = Field(
        default=None, description="Model confidence; recorded for calibration only."
    )
    evidence: Optional[str] = Field(
        default=None, description="Concise visible evidence the model cited."
    )
    action_type: Optional[str] = Field(
        default=None, description="Proposed action type, or None when absent."
    )
    action_target: Optional[str] = Field(
        default=None, description="Proposed action target, or None when absent."
    )
    action_present: bool = Field(
        description="Whether the model proposed an action on the same response."
    )
    foreground_package: Optional[str] = Field(
        default=None, description="Foreground package on the captured screen."
    )
    authority_package: Optional[str] = Field(
        default=None, description="Bound TargetAuthority package, or None."
    )
    ...
    schema_malformed: bool = Field(
        default=False, description="Whether the assessment payload failed its schema."
    )
    missing: bool = Field(
        default=False, description="Whether an observed goal produced no assessment at all."
    )
    raw_false_positive: bool = Field(
        description="Verdict SATISFIED while the oracle says not satisfied."
    )
    raw_false_negative: bool = Field(
        description="Verdict withheld while the oracle says satisfied."
    )
    admitted: bool = Field(
        description="Whether the effective shadow rule would advance the goal this attempt."
    )
    effective_false_positive: bool = Field(
        description="Admitted advance while the oracle says not satisfied."
    )
    effective_false_negative: bool = Field(
        description="Not admitted while the oracle says satisfied."
    )
    latency_ms: float = Field(
        ge=0.0, description="Wall-clock latency of the single production call."
    )
...
class CaseReport(SealedModel):
    ...
    provenance: NonBlank = Field(
        description="Source of the pixels (trace run path or a controlled tag)."
    )
    label_source: LabelSource = Field(description="How this case's ground-truth was labeled.")
    critical_negative: bool = Field(
        description="Whether an effective false positive here blocks cutover."
    )
    expected_verdict: VisualVerdict = Field(description="Oracle verdict.")
    expected_admission: bool = Field(
        description="Whether the effective rule should advance this case."
    )
    ...
    raw_false_positive: int = Field(
        ge=0, description="Repetitions with a raw verdict false positive."
    )
    raw_false_negative: int = Field(
        ge=0, description="Repetitions with a raw verdict false negative."
    )
    effective_false_positive: int = Field(
        ge=0, description="Repetitions admitted to advance against the oracle."
    )
    effective_false_negative: int = Field(
        ge=0, description="Repetitions wrongly not admitted for a satisfied goal."
    )
    missing: int = Field(ge=0, description="Repetitions with no assessment for an observed goal.")
    schema_failures: int = Field(
        ge=0, description="Repetitions whose assessment failed its schema."
    )
        ...
        *,
        ...
        return cls(
            ...
            latency=LatencySummary.from_samples(
                samples=[attempt.latency_ms for attempt in attempts]
            ),
            ...
            effective_false_positive=sum(
                1 for attempt in attempts if attempt.effective_false_positive
            ),
            effective_false_negative=sum(
                1 for attempt in attempts if attempt.effective_false_negative
            ),
...
class Totals(SealedModel):
    ...
    effective_false_positive: int = Field(
        ge=0, description="Total effective false-positive advancements."
    )
    effective_false_negative: int = Field(
        ge=0, description="Total effective false-negative retentions."
    )
...
class EvaluationReport(SealedModel):
    ...
    provenances: Tuple[str, ...] = Field(
        description="Distinct pixel sources covered by the corpus."
    )
    ...
    latency_budget_ms: float = Field(
        gt=0.0, description="The p95 latency budget the gate is held to."
    )
    ...
    def assemble(
        ...
        *,
        ...
        critical_fp = sum(case.effective_false_positive for case in cases if case.critical_negative)
        unresolved = tuple(
            case.name
            for case in cases
