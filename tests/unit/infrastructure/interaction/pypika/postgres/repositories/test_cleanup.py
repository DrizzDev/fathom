...
class _FakeUnit:
    ...
    @asynccontextmanager
    async def session(self) -> AsyncGenerator[_FakePostgresConnection, None]:
        """
...
class _FakePostgresContext:
    """
    Minimal Postgres context: passes datetimes through and exposes the unit.
    """
    ...
    def _time(self, *, value: datetime) -> datetime:
        """
        Pass datetimes through unchanged (Postgres-native timestamptz path).
        """
...
class _CleanupServiceTestBase(unittest.IsolatedAsyncioTestCase):
    """
    Shared scaffolding for the Postgres cleanup service test classes.
    """
    ...
    ) -> Tuple[PostgresCleanupService, _FakePostgresConnection]:
        """
        Construct a cleanup service wired to a recording connection.
        """

        connection = _FakePostgresConnection(responses=responses)
        context = _FakePostgresContext(connection=connection)

        return PostgresCleanupService(context=context), connection


class TestPostgresCleanupServiceNoOp(_CleanupServiceTestBase):
    """
...
class TestPostgresCleanupServiceRequestsScope(_CleanupServiceTestBase):
    """
    Covers the idempotency-requests retention sweep branch.
    """

    async def test_single_statement_tuple_in_with_tenant_filter(self) -> None:
        """
        One DELETE binds expires_at + tenant + limit inside a tuple-IN subquery.
        """
        ...
        self.assertEqual(
            ...
            "(SELECT tenant,key FROM requests WHERE expires_at<$1 AND tenant=$2 "
            "ORDER BY tenant,key LIMIT $3)",
        )
        self.assertEqual(params, (before, "acme", 500))
    ...
    async def test_global_sweep_omits_tenant_predicate(self) -> None:
        ...
        self.assertEqual(
            sql,
            "DELETE FROM requests WHERE (tenant,key) IN "
            "(SELECT tenant,key FROM requests WHERE expires_at<$1 "
            "ORDER BY tenant,key LIMIT $2)",
        )
        self.assertEqual(params, (before, 10))


class TestPostgresCleanupServiceTerminalJobsScope(_CleanupServiceTestBase):
    """
    ...
    async def test_select_includes_all_three_terminal_states(self) -> None:
        ...
        self.assertEqual(
            ...
            "(SELECT tenant,id FROM jobs WHERE state IN ($1,$2,$3) AND updated_at<$4 "
            "AND tenant=$5 ORDER BY tenant,id LIMIT $6)",
        )
        self.assertEqual(
            params,
            ("completed", "failed", "abandoned", before, "acme", 100),
        )
...
class TestPostgresCleanupServiceEventsScope(_CleanupServiceTestBase):
    """
    ...
    async def test_select_filters_on_created_threshold(self) -> None:
        """
        The subquery must filter on `created_at < $1` and the DELETE must use
        ...
        self.assertEqual(
            ...
            "(SELECT tenant,id FROM events WHERE created_at<$1 AND tenant=$2 "
            "ORDER BY tenant,id LIMIT $3)",
        )
        self.assertEqual(params, (before, "acme", 200))


class TestPostgresCleanupServiceSoftDeletedThreads(_CleanupServiceTestBase):
    """
...
class TestPostgresCleanupServiceSoftDeletedTasks(_CleanupServiceTestBase):
    """
...
class TestPostgresCleanupServiceSoftDeletedMessages(_CleanupServiceTestBase):
    """
...
class TestPostgresCleanupServiceSoftDeletedArtifacts(_CleanupServiceTestBase):
    """
    ...
    async def test_artifact_purge_single_statement_tuple_in(self) -> None:
        ...
        self.assertEqual(
            ...
            "WHERE NOT target.deleted_at IS NULL AND target.deleted_at<$1 "
            ...
            "AND target.tenant=$2 ORDER BY target.tenant,target.id LIMIT $3)",
        )
        self.assertEqual(artifacts_params, (before, "acme", 10))


class TestPostgresCleanupServiceCombinedScopes(_CleanupServiceTestBase):
    """
...
class TestPostgresCleanupServiceInjectionSafety(_CleanupServiceTestBase):
    """
    Confirms that hostile tenant strings stay parameterized.
    """
