...
class RuntimeConfigLoaderTest(unittest.TestCase):
    ...
    def test_perception_threads_inline_google_credentials_dict(self) -> None:
        """
        Inline ``GOOGLE_APPLICATION_CREDENTIALS_JSON`` material rides into
        :class:`DocumentAiCredentials` so the Document AI adapter
        authenticates against the same identity Gemini already uses —
        no reliance on ambient ADC.
        """
    ...
    def test_localization_enabled_flag_with_empty_member_list_returns_empty_tuple(self) -> None:
        """
        Enabling the flag with an explicit empty member list yields an
        enabled config with an empty tuple — the default member set is
        only chosen when the operator leaves the env value at its
        bring-up default.
        """
