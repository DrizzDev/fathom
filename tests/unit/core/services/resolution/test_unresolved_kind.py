...
class ResolutionUnresolvedKindTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_high_iou_textless_view_snaps_label(self) -> None:
        """
        Text less View whose bounds the LLM bbox overlaps (IoU above the
        agreement floor) must trust the label_id and resolve via snap —
        avoiding an unnecessary perception round-trip when the LLM and
        manifest already agree on the region.
        """
